...
def count_tokens(text, model_name="llama3-70b-8192"):
    try:
        encoding = tiktoken.get_encoding("cl100k_base")
        return len(encoding.encode(text))
    except Exception as e:
        print(f"Error counting tokens: {e}. Using fallback method.")
        return int(len(text.split()) * 1.3)  # Fallback approximation
    
def summarize_conversation(messages):
    conversation_text = "\n".join([f"{msg['role']}: {msg['content']}" for msg in messages])
    prompt = (
        "Summarize this conversation history, retaining key details like names, decisions, and context. "
        "Keep it under 150 tokens. Output only the summary.\n\n"
        f"{conversation_text}"
    )
    try:
...
def generate_response(message):
    ...
    # Build system prompt (without conversation history!)
    ...
    # Summarize if tokens exceed 5000
    if token_count > 5000:
        print(f"Token count {token_count}. Summarizing...")
        keep_last = 3  # Keep last 3 messages for context
        if len(conversation_history) > keep_last:
            to_summarize = conversation_history[:-keep_last]
            remaining = conversation_history[-keep_last:]
            summary = summarize_conversation(to_summarize)
            conversation_history = [
                {"role": "assistant", "content": f"[Summary]: {summary}"}
            ] + remaining

    # Regenerate full_messages after summarization
    full_messages = [{"role": "system", "content": system_prompt}] + conversation_history
